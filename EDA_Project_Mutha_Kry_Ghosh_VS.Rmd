...
We generated some summary statistics of variables like the installed capacity and annual generation per region (Table 1), pollutants and their $CO_2$ equivalents per region (Table 2), and lastly about the distribution of annual generation across different renewable and non-renewable sources of energy (Table 3). These tables are available in the Appendix.
...
summary.plants <- egrid_subset %>%
  ...
  rename("Nameplate capacity (GW)" = STNAMEPCAP,
         "Annual net generation (GWh)" = STNGENAN) %>%
  pivot_longer(cols = c("Nameplate capacity (GW)",
                        "Annual net generation (GWh)"),
...
summary.pollution <- egrid_subset %>%
  ...
  rename("Annual NOx emissions" = STNOXAN,
         "Annual SO2 emissions" = STSO2AN,
         "Annual CO2 emissions" = STCO2AN,
         "Annual CO2 equivalent emissions" = STCO2EQA) %>%
  pivot_longer(cols = c("Annual NOx emissions",
...
#summary stats of region-wise breakdown of annual net generation (in GWh) by fuel type
summary.fuels <- egrid_subset %>%
  ...
  rename("Coal" = STGENACL,
         ...
         "Solar" = STGENASO,
         "Geothermal" = STGENAGT,
         "Other fossil" = STGENAOF) %>%
  pivot_longer(cols = c("Coal",
                        "Oil",
                        "Gas",
                        "Nuclear",
                        "Hydro",
...
summary.table.pollution <- summary.pollution %>%
  ...
  tab_header(
    title = md("Table 2: Summary Statistics of pollutants emitted per Region (in 1000 tons)")) %>%
...
```{r Q1plot, echo=FALSE, fig.align='center', fig.cap="Distribution of Energy Sources by Region in 2021", fig.width=6.5}
energy.sources.plot
```
\newpage
...
renewable_energy_plot<-
  ...
  summarise("Hydro-electric" = sum(STGENAHY),
            "Biomass" = sum(STGENABM),
            "Wind" = sum(STGENAWI),
            "Solar" = sum(STGENASO),
            "Geothermal" = sum(STGENAGT)) %>%
  pivot_longer(cols = c("Hydro-electric",
                        "Biomass",
                        "Wind",
                        "Solar",
                        "Geothermal"),
...
```{r Q2plot, echo=FALSE, fig.cap="Distribution of different renewable energy generation sources across BEA regions of USA in 2021", fig.align='center'}
renewable_energy_plot
```
\newpage
...
In order to determine the greenhouse gas emissions across BEA regions in the US from electricity generators, we generated the stacked barplot below which shows the $CO_2$ and $CO_2$-equivalents emissions for the different regions in the US. We see that the South-East region has the highest emissions which can be correlated with the fact that this region has the greatest fraction of its electricity coming from non-renewable sources. New England and Non-Contiguous regions have the lowest emissions because of their low annual electricity generation. 
...
```{r Q3plot, echo=FALSE, fig.cap="Plot of total CO2 and CO2e emissions across BEA regions of USA in 2021", fig.align='center', fig.width=6.5}
pollutants.plot
```
...
```{r Q4plot_CO2, echo=FALSE, fig.align='center', fig.cap="Carbon dioxide emissions in each BEA region in the USA in 2021"}
CO2_Plot
```

```{r Q4plot_SO2, echo=FALSE, fig.align='center', fig.cap="Sulfur dioxide emissions in each BEA region in the USA in 2021"}
SO2_Plot
```

```{r Q4plot_NOX, echo=FALSE, fig.align='center', fig.cap="Nitrogen oxide emissions in each BEA region in the USA in 2021"}
NOX_Plot
```

```{r Q4plot_CO2e, echo=FALSE, fig.align='center', fig.cap="Carbon dioxide equivalent emissions in each BEA region in the USA in 2021"}
CO2EQA_Plot
```

\newpage
#### Question 5: Is there correlation between energy generation and pollutants emitted? 
...
```{r Q5_CO2, echo=FALSE, fig.align='center', fig.cap="Linear Regression plot of Annual Net Energy Generation and CO2 emissions"}
...
```{r Q5_CO2e, echo=FALSE, fig.align='center', fig.cap="Linear Regression plot of Annual Net Energy Generation and CO2e emissions"}
...
```{r Q5_SO2, echo=FALSE, fig.align='center', fig.cap="Linear Regression plot of Annual Net Energy Generation and SO2 emissions"}
...
```{r Q5_NOX, echo=FALSE, fig.align='center', fig.cap="Linear Regression plot of Annual Net Energy Generation and NOX emissions"}
...
```{r model1 results, echo=FALSE, fig.align='center', fig.cap="Plot showing the correlation between explanatory and response variables for Model 1"}
summary(model1)
model1.corplot
```
...
```{r model2 results, echo=FALSE, fig.align='center', fig.cap="Plot showing the correlation between explanatory and response variables for Model 2"}
summary(model2)
model2.corplot
```

Using the rank-transformed variables in our model, we get an F-statistic of 14.6 on 5 variables and 45 degrees of freedom, corresponding to a p-value <0.001. This means we can reject the null hypothesis for this model and conclude it has some explanatory power. Looking at the R2 value, we see these explanatory variables can explain under 62% of the variance of our response variable - death rate.

Since this R2 is lower than that of Model 1, this model is not as good at explaining the variance of our response variable. Therefore, we tried using a log-transform of our variables.
...
```{r model3 results, echo=FALSE, fig.cap="Plot showing the correlation between explanatory and response variables for Model 3"}
summary(model3)
model3.corplot
```

This third model has an F-statistic of 16.35 on 5 variables and 45 degrees of freedom, corresponding to a p-value of <0.001. This means we can reject the null hypothesis and conclude that our model has some explanatory power. The R2 for this model comes to 64.5% which is an improvement over Model 1.

Looking at our best model, we see that percent renewable energy consumption has a $\beta$ value of -8.11 at a p-value of 0.68. This p-value is greater than 0.1 which means that this variable does not significantly explain variance of death rate across the US. This observation goes against our initial assumption where we expected renewable energy consumption to significantly impact death rates across the US.
...
In conclusion, the data analytics assignment on electricity generation, renewable energy sources, greenhouse gas emissions, and pollutants in the BEA regions of the United States provides valuable insights that can inform policy decisions related to reducing greenhouse gas emissions, transitioning to cleaner energy sources, and identifying potential investment opportunities in renewable energy infrastructure. By analyzing the distribution of electricity generation from renewable and non-renewable energy sources, we gain a better understanding of power production in different regions of the US and the types of energy sources used, which can aid policymakers and analysts in identifying potential opportunities to increase the use of renewable energy sources and decrease reliance on non-renewable sources that contribute to greenhouse gas emissions.

Additionally, by analyzing the distribution of electricity generation from each type of renewable energy source across BEA regions, we gain insights into how different regions in the US are utilizing various renewable energy sources for electricity generation. This information is useful for identifying areas where investments in renewable energy infrastructure and technology may be needed, for developing policies and regulations that encourage the growth of specific renewable energy sources, and for evaluating potential opportunities for growth and expansion of renewable energy plants into regions that have high potential. The analysis of greenhouse gas emissions and pollutants from electricity generators across the BEA regions of the US provides insights into the sources and levels of emissions in each region, which can help in the development of strategies for reducing emissions and transitioning to cleaner energy sources.
...
```{r summary.table.plants, echo=FALSE, fig.align='center'}
summary.table.plants
```
\newpage

```{r summary.table.pollution, echo=FALSE, fig.align='center'}
summary.table.pollution
```
\newpage

```{r summary.table.fuels, echo=FALSE, fig.align='center'}
summary.table.fuels
```
